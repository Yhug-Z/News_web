...
function makeHeader(news_username, username){
    //用户信息url
    if (username){
        var user_info_url = '/' + news_username + '/' + 'info'
    }else{
        //没有登陆状态直接去登陆
    ...
    header_body += '<div class="menu" style="background-color: black;">';
    header_body += ' <nav class="nav" id="topnav"> ';
    header_body += '<h1 class="logo"><a href="/index">今日要闻</a></h1>';
    header_body += '<li><a href="/index">首页</a></li>';
    header_body += '<li><a href="/politics">时政</a></li>';
    header_body += '<li><a href="/finance">财经</a></li>';
    header_body += '<li><a href="/tec">科技</a></li>';
    header_body += '<li><a href="/sport">体育</a></li>';
    header_body += '<li><a href="/ent">娱乐</a></li>';
    header_body += '<li><a href="/house">房产</a></li>';
    header_body += '<li><a href="/auto">汽车</a></li>';
    header_body += '<li><a href="/internet">互联网</a></li>';
    header_body += '</nav>';
    header_body += '</div>';

    if(username){
        header_body += '<a href="/' + username + '/change_info" id="login" target="_blank">编辑</a>';
        header_body += '<li><span id="login_out" target="_blank">登出</span></li>';

    }
    else{
        header_body += '<a href="/login" id="login" target="_blank">登录</a>';
...
function loginOut(){
    ...
    $('#login_out').on('click', function(){
            ...
            if(confirm("确定登出吗？")){
                window.localStorage.removeItem('news_token');
